...
echo "Checking locally for required image: $FULL_IMAGE_NAME:$DOCKER_VERSION..."
if [ "$(docker images -q $FULL_IMAGE_NAME:$DOCKER_VERSION 2> /dev/null)" == "" ]; then
    echo "Image not found locally... Checking Docker Hub."
    if [ ! $(curl --silent -f -lSL https://index.docker.io/v1/repositories/$FULL_IMAGE_NAME/tags/$DOCKER_VERSION 2> /dev/null) ]; then
        echo "Image $FULL_IMAGE_NAME:$DOCKER_VERSION not found on DockerHub."
        ...
        echo "Image found on Docker Hub! Pulling $FULL_IMAGE_NAME:$DOCKER_VERSION from Docker Hub."
        docker pull $FULL_IMAGE_NAME:$DOCKER_VERSION
    fi
    
fi

echo "Running a build with container $FULL_IMAGE_NAME:$DOCKER_VERSION..."
docker run \
    --rm \
    --net=host \
    --cap-add=NET_ADMIN \
    ...
    $FULL_IMAGE_NAME:$DOCKER_VERSION \
